...
// GET USER'S TRAILER REVIEWS - STARS FOR 1
...
// GET USER REVIEWS - ALL
export const GetUserReviews = async (req: Request, res: Response, next: NextFunction) => {
  const { user } = req.params
  const trailerReview = await Reviews.find({user}).populate({path: 'trailerId', populate: [{path: 'reviews'}, {path: 'channel'}]})
  if (trailerReview[0] != null) {
    return res.status(200).json({"message": "Review Fetched", "trailerReview": trailerReview})
  } else {
